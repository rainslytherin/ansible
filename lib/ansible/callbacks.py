...
if constants.DEFAULT_LOG_PATH != '':
    # constants中为配置解析的结果，如果默认日志文件不为空，则使用配置文件中的日志文件。
    path = constants.DEFAULT_LOG_PATH
    # 判断文件是否存在，且文件与目录是否都可写，使用os.access函数，如果日志没有写权限则退出
    ...
    # 使用Python 标准的logging包，格式为：
    # 2015-09-11 15:19:03,817 p=20217 u=apps |  /usr/local/bin/ansible -i hosts test -m ping
...
def log_lockfile():
    # 该函数用来创建一个文件锁，用来保证日志的多线程、多进程安全，使用fcntl库，也就是系统的flock
    # create the path for the lockfile and open it
    # tempdir = /tmp
    # path = /tmp/ansible-lock.$uid
...
class DefaultRunnerCallbacks(object):
    ...
    ''' 为Runner() API所使用的默认回调类，在回调函数未指定时使用 '''
...
class CliRunnerCallbacks(DefaultRunnerCallbacks):
    ...
    def __init__(self):
        ...
        # 初始化时options为None，ansible在参数解析完成后，将解析结果赋值到options里面。
        self.options = None
